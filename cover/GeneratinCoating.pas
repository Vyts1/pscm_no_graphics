...
begin
 ...
 //если мы только начали т.е. нижних слоев нет, то надо бы их инициализировать. т.е. заполнить данными изначальную поверхность
   if iniVar.first_time = 1 then
     ...
     begin //если в первый раз то он почистится в инициализации, а вот если не первый раз, тогда почистим тут
 ...
 //это счетчик сплэтов включая отброшенные. начинаем с 1 для каждого слоя.
 fullcaunter := 1;
...
//    log('98');
 ...
 // }
  ...
  if iniVar.stoh = 9 then
   ...
   //эта метка делает большой цикл для фильтрации частиц которые отскакивают.
   //mogic_real генерирует частицы, они добавляются в фильтр массив fild_impact
   //и там накапливаются, и только если они там накопились, тогда проходим дальше за goto STK
   //а если не накопились, возвращаемся сюда
   STK:
...
//              crkl_volume : real;
   ...
   estestveniy_otbor := false;
   //прогнать цикл estestveniy_otbor := true если все попадает...

   //время проверять естественный отбор: если тут true, значит не прошла отбор и возвращаемся в STK
   if estestveniy_otbor then goto STK;
...
//  tipa_Rp:=step(8,1/3);
  ...
  for j:=0 to vrb.crkl_Rp_1 do
    begin
      for i:=Low(shablon_array[j]) to High(shablon_array[j]) do
        begin
          //ShowMessage('vrb.crkl_Rs_1: '+inttostr(vrb.crkl_Rs_1));
